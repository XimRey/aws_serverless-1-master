...
const emailved = async (html, csvData) => {
  const transporter = nodemailer.createTransport({
    SES: ses,
  });

  const params = {
    from: 'comunicaciones.sistemas@fastmoda.com.co 🇻🇪 Venta_Lili_Venezuela',
    // to: ['yeimy.jimenez@fastmoda.com.co',
    // 'kevin.castillo@fastmoda.com.co',
    // 'andres.hernandez@fastmoda.com.co',
    // 'david@lilipink.com', 
    // 'jazuaje@lilipink.com.ve', 
    // 'vmontiel@gina.com.ve'],
    to: ['yeimy.jimenez@fastmoda.com.co'], 
...
exports.emailved = async () => {
  const { html, csvData } = await tableEmailAccumulated();
  await emailved(html, csvData);
