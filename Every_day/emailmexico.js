...
const sendErrorEmail = async (error) => {
  ...
  const params = {
    from: 'comunicaciones.sistemas@fastmoda.com.co 🇲🇽 Venta_Lili_Mexico',
    ...
    FROM (
        ...
        FROM (
            SELECT
                ...
                ROUND(SUM(
                    CASE 
                        WHEN pcat.name IN ('Cupones', 'PROMOCIONES', 'SERVICIOS / CERTIFICADOS CLUB BI', 'TARJETAS DE REGALO') 
                        THEN 0 
                        ELSE pol.qty 
                    END
                ), 2) AS "Unid 2025",
            ...
            JOIN "odoomexico"."product_product" pp ON pol.product_id = pp.id
            JOIN "odoomexico"."product_template" pt ON pp.product_tmpl_id = pt.id
            JOIN "odoomexico"."product_category" pcat ON pt.categ_id = pcat.id
...
const emailmx = async (html, csvData) => {
  const transporter = nodemailer.createTransport({
    SES: ses,
  });
