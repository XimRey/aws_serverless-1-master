  ...
  const query = `
      ...
      FROM
          (
              ...
              UNION ALL
                SELECT 
                  tbl.bussiness_units AS "Stores",
                    ru.id as "IdUser",
                    ROUND(
                        SUM(
                            CASE 
                                WHEN sol.x_studio_moneda = 'USD' THEN sol.untaxed_amount_invoiced / so.currency_rate 
                                ELSE sol.untaxed_amount_invoiced
                            END
                        ), 2
                    ) AS "Dia 2024 (GTQ)",
                    SUM(CASE
                            WHEN pc.name LIKE '%CR-Puntos%'  
                            OR  pc.name LIKE '%Cupones%'  
                            OR  pc.name LIKE '%PROMOCION%'  
                            OR  pc.name LIKE '%SERVICIOS / CERTIFICADOS CLUB BI%'  
                            OR  pc.name LIKE '%Servicios gastos de operación%'  
                            OR  pc.name LIKE '%TARJETAS DE REGALO%'  
                            OR  pc.name LIKE '%TARJETAS MERCADERO%' 
                            THEN 0 ELSE sol.qty_invoiced
                        END
                    ) AS "Unid 2025"
                    
                FROM 
                    odooguatemala.sale_order so
                JOIN odooguatemala.sale_order_line sol ON so.id = sol.order_id
                JOIN odooguatemala.product_product pp ON sol.product_id = pp.id
                JOIN odooguatemala.product_template pt ON pp.product_tmpl_id = pt.id
                JOIN odooguatemala.product_category pc ON pt.categ_id = pc.id
                LEFT JOIN odooguatemala.res_users ru ON so.create_uid = ru.id
                LEFT JOIN odooguatemala.res_partner rp ON ru.partner_id = rp.id
                LEFT JOIN odooguatemala.general_users tbl ON ru.id= tbl.create_uid
                WHERE 
                    so.date_order BETWEEN TIMESTAMP '${format(iniDatePastYear, 'yyyy-MM-dd')} 05:00:00' AND TIMESTAMP '${format(finDatePastYear, 'yyyy-MM-dd')} 04:59:59'
                    AND so.state IN ('sale', 'done')
                GROUP BY tbl.bussiness_units, ru.id 
      ...
      FULL OUTER JOIN
          (
            ...
            FROM odooguatemala.sale_order so
            JOIN odooguatemala.sale_order_line sol ON so.id = sol.order_id
            JOIN odooguatemala.product_product pp ON sol.product_id = pp.id
            JOIN odooguatemala.product_template pt ON pp.product_tmpl_id = pt.id
            JOIN odooguatemala.product_category pc ON pt.categ_id = pc.id
            ...
            LEFT JOIN odooguatemala.general_users tbl ON ru.id= tbl.create_uid
            WHERE 
                so.date_order BETWEEN TIMESTAMP '${format(iniDate, 'yyyy-MM-dd')} 05:00:00' AND TIMESTAMP '${format(finDate, 'yyyy-MM-dd')} 04:59:59'
                AND so.state IN ('sale', 'done')
            GROUP BY tbl.bussiness_units, ru.id 
...
const tableEmailAccumulated = async () => {
  ...
  // Sumar valores
  data.forEach(item => {
    totals["Dia 2024 (GTQ)"] += parseFloat((item["Dia 2024 (GTQ)"] || "0").replace(/,/g, '')) || 0;
    totals["Unid 2024"] += parseInt((item["Unid 2024"] || "0").replace(/,/g, '')) || 0;
    totals["Dia 2025 (GTQ)"] += parseFloat((item["Dia 2025 (GTQ)"] || "0").replace(/,/g, '')) || 0;
    totals["Unid 2025"] += parseInt((item["Unid 2025"] || "0").replace(/,/g, '')) || 0;
    totals["Dia 2024 (USD)"] += parseFloat((item["Dia 2024 (USD)"] || "0").replace(/,/g, '')) || 0;
    totals["Dia 2025 (USD)"] += parseFloat((item["Dia 2025 (USD)"] || "0").replace(/,/g, '')) || 0;
  });

  // Calcular crecimiento global
  totals["% Crec Vta (USD)"] = (
  ...
  totals["% Crec Unid"] = (
    ...
    html += `<td style="padding:10px;border:1px solid #000;color:#000; text-align: right;">${totals["% Crec Vta (USD)"]}%</td>`;
  ...
  function createTotalHTMLTable(totals, title, iconUrl) {
    ...
    // Crear encabezados de la tabla 2
    html += '<th style="padding:10px;border:1px solid #000;background-color:#bfbfbf;color:#000000; font-size: 12px; text-align: center;">#</th>';
    html += '<th style="padding:10px;border:1px solid #000;background-color:#bfbfbf;color:#000000; font-size: 12px; text-align: center;">Store</th>';
    html += '<th style="padding:10px;border:1px solid #000;background-color:#f1e5b5;color:#000000; font-size: 12px; text-align: center;">Dia 2024 (GTQ)</th>';
    html += '<th style="padding:10px;border:1px solid #000;background-color:#f1e5b5;color:#000000; font-size: 12px; text-align: center;">Unid 2024</th>';
  ...
  // Calcular totales para datos acumulados tabla 3 
...
const emailgt = async (html) => {
  ...
  const params = {
    ...
    //   'nassim@dvpty.com',
    //   'juan@dvpty.com',
    //   'direccioncomercial@retailguate.com.gt',
    //   'dany.pardo@fastmoda.com.co',
    //   'alejandro.mosquera@fastmoda.com.co',
    //   'analistacomercial@retailguate.com.gt'],
    to: ['yeimy.jimenez@fastmoda.com.co', 'sebastian.rodriguez@fastmoda.com.co'],
