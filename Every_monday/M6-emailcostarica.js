  ...
  const query = `
       SELECT 
          ...
          --2024
              ...
              WHERE T0."DocDate" BETWEEN date '${format(iniDatePastYear, 'yyyy-MM-dd')}' AND TIMESTAMP '${format(finDatePastYear, 'yyyy-MM-dd')}' AND T1."AcctCode" IN ('41209501', '41209504', '41209505', '41359501', '41750201', '41750301') GROUP BY T1."ocrcode", B1."col1"
          UNION ALL
              SELECT 
                  T1."ocrcode" AS "CentroBeneficio",    
                  B1."col1" AS "Stores",
          ...
          UNION ALL
              ...
              WHERE T0."RefDate" BETWEEN date '${format(iniDatePastYear, 'yyyy-MM-dd')}' AND TIMESTAMP '${format(finDatePastYear, 'yyyy-MM-dd')}' AND T1."Account" IN ('41209501', '41209504', '41209505', '41359501', '41750201', '41750301') AND T0."TransType" = '30' GROUP BY T1."ProfitCode", B1."col1"
          UNION ALL    
          --2025
              SELECT 
                  T1."ocrcode" AS "CentroBeneficio",    
              ...
              INNER JOIN "sap-b1-lili-pink-cr"."INV1" T1 ON T0."DocEntry" = T1."DocEntry"
              LEFT JOIN "sap-b1-lili-pink-cr"."business_units" B1 ON T1."ocrcode" = B1."col0"
              WHERE T0."DocDate" BETWEEN date '${format(iniDate, 'yyyy-MM-dd')}' AND TIMESTAMP '${format(finDate, 'yyyy-MM-dd')}' AND T1."AcctCode" IN ('41209501', '41209504', '41209505', '41359501', '41750201', '41750301') GROUP BY T1."ocrcode", B1."col1"
          UNION ALL
              SELECT 
                  T1."ocrcode" AS "CentroBeneficio",    
                  B1."col1" AS "Stores",
          ...
          UNION ALL
              ...
              INNER JOIN "sap-b1-lili-pink-cr"."JDT1" T1 ON T0."TransId" = T1."TransId"
              LEFT JOIN "sap-b1-lili-pink-cr"."business_units" B1 ON T1."ProfitCode" = B1."col0"
              WHERE T0."RefDate" BETWEEN date '${format(iniDate, 'yyyy-MM-dd')}' AND TIMESTAMP '${format(finDate, 'yyyy-MM-dd')}' AND T1."Account" IN ('41209501', '41209504', '41209505', '41359501', '41750201', '41750301') AND T0."TransType" = '30' GROUP BY T1."ProfitCode", B1."col1"
          ) AS combined
          GROUP BY 
          CASE WHEN "CentroBeneficio" IN ('BOD001', 'ECOM102', 'EXP102', 'Vta_May') THEN 'CANAL MODERNO'
              WHEN "Stores" = '' OR "Stores" IS NULL THEN 'CANAL MODERNO' 
              ELSE "Stores" END 
              ORDER BY "Stores";
  `;
...
const tableEmailAccumulated = async () => {
  ...
  const { columns: columnsAccumulated, data: dataAccumulated } = await executeSQLQuery(ini_month, primer_dia_mes_siguiente, conversionRate, true);
  const { columns: columnsDaily, data: dataDaily } = await executeSQLQuery(yesterday, today, conversionRate, false);
...
const emailcrM6 = async (html) => {
  const transporter = nodemailer.createTransport({
    SES: ses,
  });

  const params = {
    from: 'cristian.supelano@fastmoda.com.co 🇨🇷 Venta_Lili_Costa_Rica',
    ...
    to: ['cristian.supelano@fastmoda.com.co','yeimy.jimenez@fastmoda.com.co'], 
...
exports.emailcrM6 = async () => {
  const { html } = await tableEmailAccumulated();
  await emailcrM6(html);
