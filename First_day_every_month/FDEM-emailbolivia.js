...
const executeSQLQuery = async (startDate, endDate) => {
  ...
  const query = `
      ...
      q1 AS (
          SELECT
              ...
              CASE
                  WHEN ac.create_uid IN (12, 2) THEN 'SANTA CRUZ'
                  WHEN ac.create_uid IN (13, 9) THEN 'COCHABAMBA'
                  WHEN ac.create_uid IN (6, 7) THEN 'LA PAZ'
                  WHEN ac.create_uid IN (20) THEN 'VENTURA MALL'
              END AS "store",
              ROUND(SUM(ac.price_subtotal), 2) AS "sales",
              COALESCE(ROUND(SUM(CASE WHEN ac.create_date BETWEEN TIMESTAMP '${format(startDate, 'yyyy-MM-dd')} 05:00:00' AND TIMESTAMP '${format(endDate, 'yyyy-MM-dd')} 04:59:59' THEN ac.qty END)), 0) AS "sold_units",
      ...
      q2 AS (
          SELECT
              ...
              COALESCE(ROUND(SUM(CASE WHEN ac.create_date BETWEEN TIMESTAMP '${format(startDate, 'yyyy-MM-dd')} 05:00:00' AND TIMESTAMP '${format(endDate, 'yyyy-MM-dd')} 04:59:59' THEN ac.qty END)), 0) AS "sold_units_ly",
      ...
      FROM stores_cte
      LEFT JOIN q1 ON stores_cte."stores" = q1."stores"
      LEFT JOIN q2 ON stores_cte."stores" = q2."stores"
      GROUP BY q1."store"
   `;
  ...
  const data = result.map((item, index) => {
    const salesbob = parseFloat((item.sales || "0").replace(/,/g, '')).toFixed(2);
    const salesUsd = parseFloat((salesbob * conversionRate).toFixed(2)).toString();
...
const sendErrorEmail = async (errorMessage) => {
  ...
  const params = {
    from: 'cristian.supelano@fastmoda.com.co 🇧🇴 Venta_Lili_Bolivia',
...
const emailbvFDEM = async (html) => {
  ...
  const params = {
    from: 'cristian.supelano@fastmoda.com.co 🇧🇴 Venta_Lili_Bolivia',
    to: ['cristian.supelano@fastmoda.com.co','yeimy.jimenez@fastmoda.com.co','diego.garcia@fastmoda.com.co','kevin.castillo@fastmoda.com.co','andres.hernandez@fastmoda.com.co','asandi@lilipinkbo.com','ammyribera@lilipinkbo.com','catherine.vergel@fastmoda.com.co','nassim@dvpty.com','david@lilipink.com'],
